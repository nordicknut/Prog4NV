...
private:
	...
	void treeToVector(TreeNode<T>* p)
	{
		...
		{
			treeToVector(p->left);
			addToArray(v, p->element, this->itrIndex);
			treeToVector(p->right);
		}
	}

	void addToArray(T* v, T value, int& index)
	{
		if (index >= arraySize)
			resizeArray(v);
		v[index] = value;
	...
	//TODO: This doesn't work. If set to arraySize == 2, this crashes.
	void resizeArray(T* v)
	{
